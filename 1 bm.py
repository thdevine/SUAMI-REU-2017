...
def minimize(quo, model, ep, tol, timeout):
	...
	#initial error
	curr_error = sum([pow(quo[m] - curr_model.d_security[m].matrix[0][0], 2) for m in range(len(curr_model.d_security))])
	...
	while curr_error > tol and iterations <= timeout:
		...
		error = sum([pow(quo[m] - curr_model.d_security[m].matrix[0][0], 2) for m in range(len(curr_model.d_security))])
