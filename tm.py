...
#A stock consists of an initial value, up/mid/down factor, and RNPs. Note that a security also has a bank it tries to beat.
class Security(object):
	def __init__(self, bank, s_0, u, m, d):
		self.bank = bank
		self.s_0 = s_0
		self.u = u
		self.m = m
		self.d = d
		if m < bank.r + 1:
			self.rnr = (u-1-bank.r) / (2*(u-m)) 
			self.rnp = ((1+bank.r) - d - (m-d)*self.rnr) / (u-d) 
			self.rnq = (u - (1+bank.r) - (u-m)*self.rnr) / (u-d)
		else:
			self.rnr = (1+bank.r-d) / (2*(m-d))
			self.rnp = ((1+bank.r) - d - (m-d)*self.rnr) / (u-d) 
			self.rnq = (u - (1+bank.r) - (u-m)*self.rnr) / (u-d)
...
class Model(object):
	def __init__(self, bank, security, d_security, n):
		...
		security.matrix = [[[float(0) for x in range(self.n+1)] for y in range(self.n+1)] for z in range(self.n+1)]
		for i in range(self.n+1):
			for j in range(self.n+1):
				for k in range(self.n+1):
					if i + j + k <= self.n:
						security.matrix[i][j][k] = float(pow(security.d, i) * pow(security.u, j) * pow(security.m, k) * security.s_0)
					else: 
						security.matrix[i][j][k] = float(0)
		...
		for m in range(len(d_security)):
			d_security[m].matrix =  [[[float(0) for x in range(self.n+1)] for y in range(self.n+1)] for z in range(self.n+1)]
			for i in range(self.n+1):
				for j in range(self.n+1):
					for k in range(self.n+1):
						if i+j+k == d_security[m].t:
							d_security[m].matrix[i][j][k] = float(d_security[m].f(security.matrix[i][j][k], d_security[m].k)) 
						else:
							d_security[m].matrix[i][j]                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              [j] = float(0)
			...
			while time >= 0:
				for i in range(self.n+1):
					for j in range(self.n+1):
						for k in range(self.n+1):
							if i+j+k == time:
								d_security[m].matrix[i][j][k] = bank.d_factor * (security.rnp * d_security[m].matrix[i][j+1][k] + security.rnq * d_security[m].matrix[i+1][j][k] + security.rnr * d_security[m].matrix[i][j][k+1])
				time -=1
...
def minimize(quo, model, ep, tol, timeout):
	...
	curr_model = model
	#initial [u,d,r].
	curr_vect = [curr_model.security.u, curr_model.security.m, curr_model.security.d, curr_model.bank.r]
	#initial error
	curr_error = sum([pow(quo[m] - curr_model.d_security[m].matrix[0][0][0], 2) for m in range(len(curr_model.d_security))])
	#sets initial u,d,r and initial error as minimums
	min_umdr = [curr_vect[i] for i in range(len(curr_vect))]
	min_error = curr_error
	#prints initial [u,d,r].
	print('Initial [u, m, d, r]: ' + str(curr_vect))
	#prints quotes prices, initial estimated prices, and initial error.
	print('Quoted prices: ' + str(quo) + ", Initial estimated prices: " + str([curr_model.d_security[m].matrix[0][0][0] for m in range(len(curr_model.d_security))])+ ', Initial error: '+str(curr_error)+"\n")
	...
	m_epp_bank = range(len(curr_model.d_security))
	m_epp_security = range(len(curr_model.d_security))
	m_epp_d_security = range(len(curr_model.d_security))
	m_epp_model = range(len(curr_model.d_security))

	m_epm_bank = range(len(curr_model.d_security))
	m_epm_security = range(len(curr_model.d_security))
	m_epm_d_security = range(len(curr_model.d_security))
	m_epm_model = range(len(curr_model.d_security))
	...
	while curr_error > tol and iterations <= timeout:
		...
		for m in range(len(curr_model.d_security)):
			...
			u_epp_security[m] = Security(u_epp_bank[m], model.d_security[m].security.s_0, curr_model.d_security[m].security.u + ep, curr_model.d_security[m].security.m, curr_model.d_security[m].security.d)
			...
			u_epm_security[m] = Security(u_epm_bank[m], curr_model.d_security[m].security.s_0, curr_model.d_security[m].security.u - ep, curr_model.d_security[m].security.m, curr_model.d_security[m].security.d)
			...
			m_epp_bank[m] = Bank(curr_model.bank.r)
			m_epp_security[m] = Security(u_epp_bank[m], model.d_security[m].security.s_0, curr_model.d_security[m].security.u, curr_model.d_security[m].security.m + ep, curr_model.d_security[m].security.d)
			m_epp_d_security[m] = DerivSecurity(u_epp_security[m], curr_model.d_security[m].k, curr_model.d_security[m].t, curr_model.d_security[m].f)
			m_epp_model[m] = Model(u_epp_bank[m], u_epp_security[m], [u_epp_d_security[m]], curr_model.n)

			m_epm_bank[m] = Bank(curr_model.bank.r)
			m_epm_security[m] = Security(u_epm_bank[m], curr_model.d_security[m].security.s_0, curr_model.d_security[m].security.u, curr_model.d_security[m].security.m - ep, curr_model.d_security[m].security.d)
			m_epm_d_security[m] = DerivSecurity(u_epm_security[m], curr_model.d_security[m].k, curr_model.d_security[m].t, curr_model.d_security[m].f)
			m_epm_model[m] = Model(u_epm_bank[m], u_epm_security[m], [u_epm_d_security[m]], curr_model.n)

			d_epp_bank[m] = Bank(curr_model.bank.r)
			d_epp_security[m] = Security(d_epp_bank[m], curr_model.d_security[m].security.s_0, curr_model.d_security[m].security.u, curr_model.d_security[m].security.m, curr_model.d_security[m].security.d + ep)
			...
			d_epm_security[m] = Security(d_epm_bank[m], curr_model.d_security[m].security.s_0, curr_model.d_security[m].security.u, curr_model.d_security[m].security.m, curr_model.d_security[m].security.d - ep)
			...
			r_epp_security[m] = Security(r_epp_bank[m], curr_model.d_security[m].security.s_0, curr_model.d_security[m].security.u, curr_model.d_security[m].security.m, curr_model.d_security[m].security.d)
			...
			r_epm_security[m] = Security(r_epm_bank[m], curr_model.d_security[m].security.s_0, curr_model.d_security[m].security.u, curr_model.d_security[m].security.m, curr_model.d_security[m].security.d)
		...
		epp_vect = [sum([u_epp_model[m].d_security[0].matrix[0][0][0] for m in range(len(curr_model.d_security))]), sum([m_epp_model[m].d_security[0].matrix[0][0][0] for m in range(len(curr_model.d_security))]), sum([d_epp_model[m].d_security[0].matrix[0][0][0] for m in range(len(curr_model.d_security))]), sum([r_epp_model[m].d_security[0].matrix[0][0][0] for m in range(len(curr_model.d_security))])]
		epm_vect = [sum([u_epm_model[m].d_security[0].matrix[0][0][0] for m in range(len(curr_model.d_security))]), sum([m_epm_model[m].d_security[0].matrix[0][0][0] for m in range(len(curr_model.d_security))]),sum([d_epm_model[m].d_security[0].matrix[0][0][0] for m in range(len(curr_model.d_security))]), sum([r_epm_model[m].d_security[0].matrix[0][0][0] for m in range(len(curr_model.d_security))])]
		grad_mult = sum([(quo[m] - curr_model.d_security[m].matrix[0][0][0]) / ep for m in range(len(curr_model.d_security))])
		...
		while next_vect[2] < 0 or next_vect[1] < next_vect[2] or next_vect[0] < next_vect[1] or (1 + next_vect[3]) < next_vect[2] or next_vect[0] < (1 + next_vect[2]):
		...
		curr_bank = Bank(curr_vect[3])
		curr_sec = Security(curr_bank, curr_model.security.s_0, curr_vect[0], curr_vect[1], curr_vect[2])
		...
		error = sum([pow(quo[m] - curr_model.d_security[m].matrix[0][0][0], 2) for m in range(len(curr_model.d_security))])
		...
		#prints min u,d,r and min error from iterations fo far
 		print("Min error: " +str(min_error) + ", Best [u,d,r]: " + str(min_umdr)+"\n")
...
def matrix_print(matrix):
	for m in range(len(matrix)):
		two_matrix_print(matrix[m])
	print("\n\n\n")

def two_matrix_print(matrix):
...
put_options_quoted = [3.25, 0.66, 5.05]
#specify strike price for corresponding put
put_options_stike = [140, 125, 145]
...
stock = Security(bank, s_0, u_0, m_0, d_0)
